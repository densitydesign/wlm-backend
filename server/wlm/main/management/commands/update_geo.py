...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("municipalities_lookup_path", type=str)
        parser.add_argument("--dry-run", action="store_true", required=False)

    @transaction.atomic
    def handle(self, *args, **options):
        update_geo(
            options["regions_path"],
            options["provinces_path"],
            options["municipalities_path"],
            options["municipalities_lookup_path"],
        )
        update_geo_cache()
